...
;; Anki
(package! emacs-anki-helper
  :recipe (:host github :repo "Elilif/emacs-anki-helper"))
...
;;; org-capture
(after! org
  (setq org-default-notes-file (expand-file-name "tasks.org" org-roam-directory))
...
(after! org-agenda
  ...
  (map! :leader
        ...
        "od" (cmd! (org-agenda nil "d"))))

(after! org
  (setq org-agenda-files '("~/Documents/emacs/org/roam/projects.org"
                           "~/Documents/emacs/org/roam/archive.org"
                           "~/Documents/emacs/org/roam/tasks.org"))
...
(after! elfeed
  ...
  ;; https://github.com/skeeto/elfeed/issues/392#issuecomment-884857504
...
(defun ascii-progress-bar (progress)
  "Generate an ASCII progress bar with percentage based on PROGRESS (0.0 to 1.0)."
  (let* ((bar-width 30)
...
(defun insert-progress-bar ()
  "Prompt for a progress value and insert an ASCII progress bar at point."
  (interactive)
  (let* ((input (read-string "Enter progress (0.0 to 1.0): "))
         (progress (string-to-number input)))
    (if (and (>= progress 0.0) (<= progress 1.0))
        (insert (ascii-progress-bar progress))
      (message "Invalid input. Please enter a number between 0.0 and 1.0."))))
#+end_src
**** 快速插入截图到文件
#+begin_src emacs-lisp :tangle "~/.doom.d/config.el"
...
(defun nasy/frame-recenter (&optional target-frame)
  "Center TARGET-FRAME on the screen.
TARGET-FRAME can be a frame name, a terminal name, or a frame.
If TARGET-FRAME is omitted or nil, use currently selected frame."
  (interactive)
  (unless (eq 'maximised (frame-parameter nil 'fullscreen))
    (let* ((frame (or target-frame (selected-frame)))
